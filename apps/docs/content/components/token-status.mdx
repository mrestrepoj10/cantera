...
<Badge variant="accent">component</Badge> `@cantera/token-status`

## Install

```bash
npx shadcn@latest add @cantera/token-status
```
...
<ComponentPreview src="https://canteraui.xyz/embed/token-status" name="token-status" title="Token Status preview" />

## Files

| Path | Type |
| --- | --- |
| `token-status.tsx` | `registry:ui` |
...
The machine-readable item the CLI fetches: [`token-status.json`](https://canteraui.xyz/r/token-status.json).
