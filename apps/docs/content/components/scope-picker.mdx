---
title: "Scope Picker"
description: "A controlled OAuth scope picker: checkbox list with descriptions, one-click presets, required scopes pinned on, and an escape hatch for scopes outside the catalog."
---

<Badge variant="accent">component</Badge> `@cantera/scope-picker`
...
<ComponentPreview src="https://canteraui.xyz/embed/scope-picker" name="scope-picker" title="Scope Picker preview" />

## Files

| Path | Type |
| --- | --- |
| `scope-picker.tsx` | `registry:ui` |
...
| `allowCustomScopes` | `boolean` | `false` | Adds a field for scopes outside the catalog — granular resource scopes like data:read:&lt;urn>, or anything the provider added since. Custom scopes render with a "custom" badge and round-trip through value / onChange like any other. |
| `customScopeLabel` | `string` | `'Add a scope'` | Label for the custom-scope field. |
| `disabled` | `boolean` | `false` | Disables the presets, every checkbox, and the custom-scope field. |
...
The machine-readable item the CLI fetches: [`scope-picker.json`](https://canteraui.xyz/r/scope-picker.json).
