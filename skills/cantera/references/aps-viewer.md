...
- `extensions / viewerConfig` (`string[] / Record<string, unknown>`) — Extension ids and extra constructor configuration captured when the viewer mounts.
...
- `onViewerReady / onModelLoaded / onError` (`callbacks`) — Lifecycle callbacks. Inline functions do not recreate the viewer.
...
- `useAPSViewerEvent / useAPSContextMenu / useAPSExtension` (`hooks`) — Raw event, context-menu, and extension lifecycle escape hatches.
