...
**Motion grammar — exactly three moves.** (1) Icon and spinner crossfade, about 150ms ease-out. (2) Press feedback, delegated to the primitive's `active:translate-y-px`. (3) Status color transitions, via `transition-colors`. That is the whole vocabulary. No entrance animations on data-dense content — a connections list does not stagger in.
