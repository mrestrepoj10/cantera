...
- **kind** — the site-level taxonomy over `type`, carried in `meta.kind`: `template` (a `registry:block` that ships a page with routes, `envVars`, and the aec-auth glue — ready to deploy), `block` (a section or surface built from components — a sign-in card, a sidebar, a drop zone — with no routes and nothing under `app/`; may be `registry:component` or `registry:block`), or `kit` (`acc-auth-routes`, the headless auth wiring templates share). Items without a kind are components or foundations by `type`. `apps/www/lib/registry-kinds.ts` derives the four catalog groups — templates, blocks, components, foundations — from it; the site (`/components`, `/blocks`, `/templates`), skill, and llms artifacts all group that way.
- **docs policy** — only templates and the kit carry a `docs` string, each at most 120 words in "installed at / Next / reference" form, because the CLI prints every `docs` in an install's closure as one block. Component and lib notes live in `installNotes` in `components/site/props-tables.ts` and render on the docs page, the markdown twin, the skill, and llms-full.txt. `verify-docs.mts` enforces both.
...
- **the pattern** — every domain ships as generic types (a per-domain `registry:lib`, never one monolithic types item) + provider adapters (presets translating APS/ACC payloads into those types) + blocks (sections and surfaces composed from the components) + templates (ready-to-deploy pages with their routes and aec-auth glue).
...
- Generated output is committed and byte-verified. `registry:build` derives the example items, the server-only docs demo registry, `public/r/`, the llms.txt artifacts, `skills/cantera/`, and `templates/` (one deployable Next.js app per template: its install closure at the CLI's paths, the shadcn primitives it pulls, and the skeleton in `apps/www/scripts/template-skeleton/`) from `registry.json` + `components/site/props-tables.ts`; every URL comes from `apps/www/lib/site.ts`. Generators stay deterministic — no timestamps, no unordered iteration — because `registry:verify` rebuilds into a scratch directory and compares byte for byte.
