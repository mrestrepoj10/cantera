...
- **A11y bar** — 3:1 focus indicators, name/role/state on every control, `aria-describedby` wiring, real headings, locale-neutral `Intl`. Components are designed to meet WCAG A/AA; there is no automated axe gate, so when you touch an interactive surface, check these properties yourself — the docs pages render the registry sources, so what ships to consumers is what you reviewed.
