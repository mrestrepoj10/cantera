...
- **kind** — the site-level taxonomy over `type`, carried in `meta.kind` on `registry:block` items: `template` (a page with routes, `envVars`, and the aec-auth glue), `block` (a page-sized composition with no routes and no environment), or `kit` (`acc-auth-routes`, the headless auth wiring templates share). `apps/www/lib/registry-kinds.ts` derives the four catalog groups — templates, blocks, components, foundations — from it; the site, skill, and llms artifacts all group that way.
...
- **the pattern** — every domain ships as generic types (a per-domain `registry:lib`, never one monolithic types item) + provider adapters (presets translating APS/ACC payloads into those types) + blocks (page-sized compositions of the components) + templates (pages and routes mounting a block with aec-auth).
