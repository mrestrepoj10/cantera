...
- **docs policy** — only templates and the kit carry a `docs` string, each at most 120 words in "installed at / Next / reference" form, because the CLI prints every `docs` in an install's closure as one block. Component and lib notes live in `components/site/install-notes.ts` and render on the docs page, the markdown twin, the skill, and llms-full.txt. `verify-docs.mts` enforces both.
